...
bind \ch 'pgrep hoogle; or nohup stack exec -- hoogle server --local --database=.stack-work/hoogle >/tmp/hoogle.(random) &; open "http://localhost:8080"'
...
function hackage-search
    curl -H "Accept: application/json" "hackage.haskell.org/packages/search?terms=$argv" | jq '.[].name'
end
